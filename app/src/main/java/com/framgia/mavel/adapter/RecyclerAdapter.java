...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.RecyclerViewHolder> {
    ...
    private ArrayList<String> idHeroFav = new ArrayList<>();
    ...
    public void onBindViewHolder(final RecyclerViewHolder holder, final int position) {
        ...
        for (String temp : idHeroFav) {
            if (data.get(position).getId().compareTo(temp) == 0) {
        ...
        holder.imageHero.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Toast.makeText(view.getContext(),
                         "Da Click  : " + data.get(position).getNameOfHero(),
                         Toast.LENGTH_SHORT).show();
            }
        });
        final boolean finalCheckIcon = checkIcon;
        holder.imageHero.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                if (finalCheckIcon == true) {
