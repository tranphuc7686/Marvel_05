...
public class SqliteHelper {
    ...
    public void insertLikeHero(String valueIDHero) {
        ...
        if (mSqLiteDatabase.insert(TABLE_NAME_FAVORITE, null, contentValues) == -1) {
            Toast.makeText(mAppCompatActivity.getBaseContext(),
                     "Lỗi Hệ Thống ",
                    Toast.LENGTH_SHORT).show();
    ...
    public void insertDontLikeHero(String valueIDHero) {
        if (mSqLiteDatabase.delete(TABLE_NAME_FAVORITE,
                ISFAV_COLUMN + "=?",
...
//        ArrayList<String> arrHeroMarvel = new ArrayList<HeroMarvel>();
//        Cursor c = mSqLiteDatabase.query(TABLE_NAME_HERO, null, ISFAV_COLUMN + "=?", new String[]{"FAV"}, null, null, null);
//        c.moveToFirst();
//        while (!c.isAfterLast()) {
